...
if( isset( $_POST['updateSettings'] ) ) {
     ...
     $wtnSettingsInfo = array(
          'wtn_api_key' => ( sanitize_text_field( $_POST['wtn_api_key'] ) != '' ) ? sanitize_text_field( $_POST['wtn_api_key'] ) : '',
     );
...
<div id="wph-wrap-all" class="wrap">
     ...
     <?php if( $wtnShowMessage ) { $this->wtn_display_notification('success', 'Your information updated successfully'); } ?>

     <form name="wpre-table" role="form" class="form-horizontal" method="post" action="" id="wtn-settings-form">
          <table class="form-table">
          <tr class="wtn_api_key">
               <th scope="row">
                    <label for="wtn_api_key"><?php esc_html_e('API Key:', WTN_TXT_DMN); ?></label>
               </th>
               <td>
                    <input type="password" name="wtn_api_key" class="regular-text" value="<?php echo esc_attr( $wtn_api_key ); ?>" autocomplete="off">
               </td>
          </tr>
          </table>
