...
{	
	...
	*/
	function wtn_enqueue_assets(){
		...
		//if (isset($_GET['page']) && $_GET['page'] == 'wtn-settings') {
			wp_enqueue_style('wtn-admin-style', WTN_ASSETS . 'css/wtn-admin-style.css', array(), $this->wtn_version, FALSE );
		...
		wp_enqueue_script('wtn-admin-script', WTN_ASSETS . 'js/wtn-admin-script.js', array('jquery'), $this->wtn_version, true );
		$wtnAdminScriptData = array(	
										'wtnIdsOfColorPicker' => array( '#wtn_form_header_bg_color', '#wtn_form_header_font_color', '#wtn_submit_button_color' ),
										'deleteConfirm' => esc_html__( 'Are you sure you want to delete this item?' ),
									);
		wp_localize_script( 'wtn-admin-script', 'wtnAdminScript', $wtnAdminScriptData );
	}
	...
	*/
	...
	protected function wtn_display_notification($type, $msg){ ?>
		<div class="wtn-alert <?php echo $type; ?>">
